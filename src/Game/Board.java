...
public class Board {
    ...
    public void storeBoardinHistory(){
        ...
        this.history.add(new Memento(board));
    }

    public void undo(){
        if (!this.history.isEmpty()){
            Memento m = this.history.remove(this.history.size()-1);
            this.currentPosition = m.getSavedBoard();
        }
    }
    ...
    public static class Memento {
        ...
        public Memento(Spot[][] boardToSave) {
            this.board = boardToSave;
        }

        // accessible by outer class only
        private Spot[][] getSavedBoard() {
            return board;
        }
